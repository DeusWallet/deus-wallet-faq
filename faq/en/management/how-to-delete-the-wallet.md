...
To delete wallets and related data from Deus:
...
The unlink action merely deletes the private key controlling the wallet from the Deus app. To restore deleted wallets in the future, users require a private key (mnemonic phrase) for each such wallet as well as its passphrase(if it had any). Restoring access to a deleted wallet without knowing its private key (mnemonic phrase) is impossible.
