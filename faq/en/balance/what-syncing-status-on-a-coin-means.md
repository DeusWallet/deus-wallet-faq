...
The 'syncing' status indicator on the coin shows that the Deus app tries to synchronize itself with a given blockchain.
...
Keep in mind, Deus app keeps the synchronization process running only when the Deus app remains open. If the app is closed the synchronization process is paused and only resumes when the app is opened again.
